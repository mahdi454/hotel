...
function CreateCabinForm({ cabin = {},onClose }) {
  ...
  function onSubmit(data) {
    const imagee = typeof data.image === 'string' ? data.image : data.image[0]
    if (isEdit) editCabin({
      newCabin: { ...data, image: imagee }, id: editId }, {
      onSuccess: () => {
        reset();
        onClose?.()
      }
    });
    else createCabin({ ...data, image: imagee }, {
      onSuccess: () => {
        reset();
        onClose?.()
      }
    ...
    <Form onSubmit={handleSubmit(onSubmit)} type={onClose? "modal": "regular"}>
      ...
      <FormRow label='Discount' error={errors?.discount?.message}>
        <Input type="number" id="discount" defaultValue={0} {...register('discount', {
          validate: (value) => value <= getValues('regularPrice') || "Discount should be less then price"
        })} />
      </FormRow>
